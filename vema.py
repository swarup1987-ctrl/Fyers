...
class VEMAStrategy:
    ...
    def __init__(self,
                 ...
                 ema_fast=9,          # EMA fast period (tunable)
                 ema_slow=21,         # EMA slow period (tunable)
        ...
        # Fixed order for all possible params for consistent string output
        self.param_keys = [
            "stop_loss_pct", "target_pct", "vol_window", "ema_fast", "ema_slow", "daily_loss_cap"
        ]
    ...
    def params_to_str(self, params=None):
        ...
        # Ensure all keys are present in params dict for reporting
        if params is None:
            params = self._params
        # Fill missing keys from self attributes if not in params
        return ", ".join(f"{k}={params[k] if k in params else getattr(self, k, '?')}" for k in keys)
    ...
    def fit(self, train_df, param_grid=None, sampler="bayesian", n_trials=8, n_random_trials=3,
        ...
        def objective(trial):
            ...
            # Avoid degenerate fast >= slow case
        ...
        # Ensure all keys are present in best_params for reporting/logging
        for k in self.param_keys:
            if k not in best_params:
                best_params[k] = getattr(self, k, "?")
