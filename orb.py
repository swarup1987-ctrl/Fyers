...
class ParamCache:
    def __init__(self, cache_path="orb_opt_cache.pkl"):
...
class ORBStrategy:
    ...
    - Target: set at 1:2 or 1:3 risk-reward ratios (risk per trade * 2 or 3)
    - At most one trade per day
    - Any outstanding trade must exit before or at 15:15 IST of that trade day
    """
    def __init__(
        self,
        stop_type="range",  # "range" or "mid"
        rr_ratio=2.0,       # risk-reward ratio, e.g., 2.0 for 1:2, 3.0 for 1:3
    ):
        ...
        for day, day_df in df.groupby("date"):
            ...
            for i in range(len(day_df)):
                if breakout_idx is not None and i == breakout_idx:
                    ...
                    risk = abs(entry - stop)
                    tgt = entry + self.rr_ratio * risk if breakout_type == "BUY" else entry - self.rr_ratio * risk
    ...
    def backtest_on_signals(self, df, signals, disable_trailing=False):
        """
        For each day, take at most one trade: enter at next bar open after breakout, stop loss at opposite end, 
        target as per risk-reward, exit all trades before or at 15:15 IST.
        """
        df = df.copy()
        ...
        for day, day_df in df.groupby("date"):
            ...
            exit_price = None
            exit_reason = None
            trade_entry_date, _ = ist_date_and_time(trade_entry_ts)
            for j in range(sig_idx+1, len(day_df)):
                ...
                low = row["low"]
                # Any outstanding trade must exit before or at 15:15 IST of that trade day
                if ist_date != trade_entry_date or ist_time > time(15, 15):
                    # Exit at previous bar (last bar <= 15:15 IST), or at entry bar if no bars after entry
                ...
                if direction == "BUY" and high >= target:
                    ...
                    exit_reason = "Target"
                    break
            # If trade not closed, close at last bar (must also be <= 15:15 IST)
            if exit_idx is None:
                # find last bar <= 15:15 IST of trade day
                last_idx = sig_idx
                for k in range(sig_idx+1, len(day_df)):
                    tstamp = day_df.iloc[k]["timestamp"]
                    d, t = ist_date_and_time(tstamp)
                    if d == trade_entry_date and t <= time(15, 15):
                        last_idx = k
                exit_idx = day_df.index[last_idx]
                exit_price = df.loc[exit_idx, "close"]
                exit_reason = "EOD"
            size = 1
    ...
    def fit(
        ...
        n_trades_accept=1,
        cache=None,
        cache_path="orb_opt_cache.pkl",
        prev_best_params=None,
        print_callback=None,
        log_file="orb_optuna_trials.log"
    ):
        """
        ...
        """
        logger = logging.getLogger("ORBOptunaTrialLogger")
        ...
        if param_grid is None:
            param_grid = {
                "stop_type": ["range", "mid"],
                "rr_ratio": [2.0, 3.0],  # 1:2 or 1:3 RR
            }
        ...
        if sampler == "grid":
            for idx, params in enumerate(param_dicts):
                _, score, stats = ORBStrategy._evaluate_param_combo(params, train_df_dict, metric)
                msg = f"[ORB Grid Trial {idx+1}] Params={params} | {metric}: {score:.5f}"
                print(msg)
                logger.info(msg)
                if print_callback:
            ...
            for idx, params in enumerate(random_param_dicts):
                _, score, stats = ORBStrategy._evaluate_param_combo(params, train_df_dict, metric)
                msg = f"[ORB Random Trial {idx+1}] Params={params} | {metric}: {score:.5f}"
                print(msg)
                logger.info(msg)
                if print_callback:
                    print_callback(idx+1, params, score)
            ...
            def objective(trial):
                params = {
                    ...
                    "rr_ratio": trial.suggest_categorical("rr_ratio", param_grid["rr_ratio"]),
                }
                _, score, stats = ORBStrategy._evaluate_param_combo(params, train_df_dict, metric)
                msg = f"[ORB Optuna Trial {trial.number}] Params={params} | {metric}: {score:.5f}"
                print(msg)
                logger.info(msg)
                if print_callback:
            ...
            _, best_score, best_stats = ORBStrategy._evaluate_param_combo(best_params, train_df_dict, metric)
